...
SELECT
  ...
  CASE
    ...
    THEN EXTRACT(EPOCH FROM dropoff_datetime - pickup_datetime) / 60
  END AS duration_in_minutes,
  ST_GeoHash(ST_SetSRID(ST_MakePoint(pickup_longitude,pickup_latitude),4326)) AS pickup_geohash
FROM trips
...
CREATE INDEX index_trips_from_manhattan_with_geohash_geohash ON trips_from_manhattan_with_geohash_test(geohash text_pattern_ops);
...
-- STEP 3. Group by the trips from Manhattan by the first six letters of the pickup_geohash, and get the count
...
STEP 1. Similar to Case 1, but reverse the where clause.
