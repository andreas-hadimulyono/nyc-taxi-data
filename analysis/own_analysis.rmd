...
-- STEP 1. Group by the trips from Manhattan by the first six letters of the pickup_geohash, and get the count

CREATE TABLE manhattan_pickup_summary_geohash_test AS
SELECT substring(pickup_geohash, 1, 6) as pickup_geohash_prefix, COUNT(*) as pickup_count
FROM trips_from_manhattan_with_geohash_test
GROUP BY pickup_geohash_prefix;
...
CREATE INDEX index_trips_from_midtown_to_downtown_dow ON trips_from_midtown_to_downtown_test(dow);
CREATE INDEX index_trips_from_midtown_to_downtown_dow ON trips_from_midtown_to_downtown_test(hour);

      
STEP 2. Filter it further to just weekdays with dropoff hour 9, and get the date to be joined later

CREATE TABLE trips_from_midtown_to_downtown_weekday_9 AS
SELECT
  id,
  cab_type_id,
  pickup_datetime,
  ...
  dow,
  hour,
  date(dropoff_datetime) AS pickup_date,
  CASE
    WHEN EXTRACT(EPOCH FROM dropoff_datetime - pickup_datetime) / 60 BETWEEN 0.1 AND 240
    THEN EXTRACT(EPOCH FROM dropoff_datetime - pickup_datetime) / 60
  END AS duration_in_minutes
FROM trips_from_midtown_to_downtown_test
WHERE hour = 9 AND dow IN (1,2,3,4,5);

CREATE INDEX index_trips_from_midtown_to_downtown_weekday_9_pickup_date ON trips_from_midtown_to_downtown_weekday_9(pickup_date);

STEP 3. JOIN with central_park_weather_observation to find rainy/sunny days. 0 = rain, 1 = sunny

CREATE TABLE trips_from_midtown_to_downtown_weekday_9_weather AS 
SELECT 
    t.pickup_date,
...
FROM trips_from_midtown_to_downtown_weekday_9 t
    INNER JOIN central_park_weather_observation w ON t.pickup_date = w.date;
...
FROM trips_from_midtown_to_downtown_weekday_9_weather
...
3. The movie starts at 7PM and we are leaving from home, so for this case we shall compare those with origin Midtown, destination Downtown. Similar to case 1, but this time we are looking at all days instead of just week days.
...
5. We shall pick the friend's place as Crown's Heights NOrth suburb, again in Broklyn. We shall leave the parent's place at 2PM.
