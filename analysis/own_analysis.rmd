...
-- STEP 2. Find only Friday trips originating from Midtown, and then get count, mean and median
...
-- Step 1. Find all the trips originating or ending in Manhattan

CREATE TABLE trips_to_from_manhattan_with_geohash AS
...
SELECT
  ...
  ST_GeoHash(ST_SetSRID(ST_MakePoint(dropoff_longitude,dropoff_latitude),4326)) AS dropoff_geohash
FROM trips
WHERE pickup_nyct2010_gid IN (select gid from manhattan_gids)
  OR dropoff_nyct2010_gid IN (select gid from manhattan_gids);
```
